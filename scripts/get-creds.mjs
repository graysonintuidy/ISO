...
async function main() {
  await mcpPost({ jsonrpc: '2.0', id: 1, method: 'initialize', params: { protocolVersion: '2024-11-05', capabilities: {}, clientInfo: { name: 'creds', version: '1.0.0' } } });
  await mcpPost({ jsonrpc: '2.0', method: 'notifications/initialized' });

  // Get full DB details including credential_id
  console.log('=== Full DB 10 Details ===');
  const db = await callTool('get_database_status', { databaseId: 10 });
  console.log(JSON.stringify(db, null, 2));

  // Also list all databases to see credential_ids
  console.log('\n=== All DBs with credentials ===');
  const dbs = await callTool('list_managed_databases', {});
  const list = Array.isArray(dbs) ? dbs : [];
  for (const d of list) {
    console.log(`  ID: ${d.id} | cred: ${d.credential_id} | "${d.name}" | db: ${d.db_name}`);
  }
}
